 ...
 * @param status - Expected status text (e.g., 'Ready', 'Connecting', 'Disconnected')
 */
export async function waitForConnectionStatus(
  page: Page, 
  status: 'Ready' | 'Connecting' | 'Disconnected' | 'Configuring'
): Promise<void> {
...
export async function disconnectDevice(page: Page): Promise<void> {
  ...
  if (disconnectButton) {
    ...
    try {
      // Look for the modal confirmation button (it has data-testid="modal-confirm-button")
      ...
      if (confirmButton) {
        console.log('[Connection] Confirming disconnect in modal dialog');
