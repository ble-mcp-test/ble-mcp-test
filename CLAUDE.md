...
**Conformance arm B is the only check that establishes fidelity, and it is manual forever.** It runs real Chromium `navigator.bluetooth` with a human answering Chrome's chooser once per check, so it needs **any host with a real BLE radio of its own** — BlueZ over D-Bus or CoreBluetooth, and **never the bridge's host**, because the ESPHome proxy is the bridge's route to the device and Chrome cannot use it. That requirement is the durable part; which box satisfies it is not. `docs/conformance-arm-b.md` holds the per-host setup and the recorded results.

**It is timing-sensitive, so the host is part of the result.** Arm B assumes the link survives the gap between two `requestDevice()` calls, and that gap is the operator's answer plus whatever the machine adds. A green run from a saturated box is worth less than one from a fast one — say which host a result came from, always.
